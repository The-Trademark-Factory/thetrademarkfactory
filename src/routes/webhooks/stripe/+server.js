...
export const POST = async ({ request }) => {
	...
	try {
		event = await stripe.webhooks.constructEventAsync(rawRequest, sig, VITE_STRIPE_WEBHOOK_SECRET);
	} catch (err) {
	...
	switch (event.type) {
		...
		case 'payment_intent.succeeded': {
			...
			const orderRefs = await firebaseDb
				...
				.get();
			if (!orderRefs?.size)
				throw error(400, 'Order not found 3: ' + stripePaymentIntentId + ' ' + orderRefs?.size);
			...
			formData.append(
				'GST',
				new Intl.NumberFormat('us-EN', { style: 'currency', currency: 'AUD' }).format(gst)
			);

			formData.append(
				'Price total',
				new Intl.NumberFormat('us-EN', { style: 'currency', currency: 'AUD' }).format(total)
			);

			formData.append(
				'Order detail url',
				`${import.meta.env.VITE_PUBLIC_SITE_URL}/orders/${order.id}`
			);
